...
const productionDist=resolve(commissioner,"dist");
const demoDirectories=["src/application/demo","src/routes/demo","src/ui/demo"].map(path=>resolve(productionDist,path));
const demoEntryPrefix=resolve(productionDist,"src/server/demo-main");
await cp(productionDist,join(destination,"apps","commissioner","dist"),{recursive:true,filter:source=>{const path=resolve(source);return !path.startsWith(demoEntryPrefix+".")&&!demoDirectories.some(excluded=>path===excluded||path.startsWith(excluded+"\\")||path.startsWith(excluded+"/"));}});
