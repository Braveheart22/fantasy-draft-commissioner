...
import { dependencyCut, resumeState } from "./sqlite-dependency-registry.js";
import type { CorrectionPreview, CorrectionType, DependencyItem } from "../../application/corrections/correction-types.js";
import type { CommandMetadata } from "../../application/ports/season-repository.js";
import type { CorrectionConfirmation, CorrectionOperationsPort } from "../../application/corrections/correction-service.js";

const hashCut = (items: DependencyItem[]) => createHash("sha256").update(JSON.stringify(items)).digest("hex");

export class SqliteCorrectionAdapter implements CorrectionOperationsPort {
  ...
  async preview(seasonId: string, correctionType: CorrectionType, targetId: string | undefined, metadata: CommandMetadata): Promise<CorrectionPreview> {
    ...
    try {
      const duplicate = db.prepare("SELECT resultJson FROM AuditEvent WHERE seasonId=? AND actorType=? AND idempotencyKey=?").get(seasonId, metadata.actor.type, metadata.idempotencyKey) as { resultJson: string } | undefined; if (duplicate) return JSON.parse(duplicate.resultJson) as CorrectionPreview;
      ...
      if (metadata.expectedVersion !== seasonVersion) throw new Error("Stale season version");
  ...
  confirm(previewId: string, input: CorrectionConfirmation): Promise<CorrectionPreview> {
    ...
    try {
      return Promise.resolve(db.transaction(() => {
        ...
        db.prepare("INSERT INTO AuditEvent (id,seasonId,sequence,actorType,actorLabel,commandType,entityType,entityId,correlationId,idempotencyKey,reason,beforeJson,afterJson,resultJson,createdAt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)").run(auditId, row.seasonId, sequence, actor.type, actor.label, input.metadata.commandType, "CorrectionAction", previewId, input.metadata.correlationId ?? randomUUID(), idempotencyKey, input.reason, JSON.stringify({ seasonVersion: currentVersion, cutHash }), JSON.stringify({ seasonVersion: currentVersion + 1, state, superseded: manifest }), JSON.stringify({ previewId, backupHash: input.backupHash }));
