...
export interface PrismaSeasonStoreOptions {
...
function databaseErrorCode(error: unknown): string | undefined {
  ...
  if (typeof value.code === "string" && ["40001", "40P01"].includes(value.code)) return value.code;
  return databaseErrorCode(value.meta) ?? databaseErrorCode(value.cause) ?? (typeof value.code === "string" ? value.code : undefined);
}
...
export class PrismaSeasonStore implements SeasonRepository, SetupRepository, AuctionRepository, DraftOrderRepository, ConventionalDraftRepository, CatalogRepository, CatalogPreparationRepository, PricingRepository, BootstrapRepository, ResultsRepository, OperationsRepository {
  ...
  constructor(
    ...
    options: PrismaSeasonStoreOptions = {},
  ) {
    this.dialect = options.dialect ?? "sqlite";
    this.maxTransactionAttempts = options.maxTransactionAttempts ?? 3;
  }

  private commandFingerprint(metadata: CommandMetadata): string {
    if (this.dialect === "postgres" && !metadata.commandFingerprint) throw new Error("Hosted commands require a command fingerprint");
    return metadata.commandFingerprint ?? hashJson({ commandType: metadata.commandType });
  }

  private async retryPostgresTransaction<T>(work: () => Promise<T>): Promise<T> {
    for (let attempt = 1; ; attempt += 1) {
      try {
        return await work();
      } catch (error) {
        const code = databaseErrorCode(error);
        if (this.dialect !== "postgres" || !["40001", "40P01"].includes(code ?? "") || attempt >= this.maxTransactionAttempts) throw error;
        await new Promise(resolve => setTimeout(resolve, attempt * 10));
      }
    }
  }

  private async lockSeason(database: any, seasonId: string): Promise<void> {
    if (this.dialect === "postgres") {
      const rows = await database.$queryRawUnsafe('SELECT id FROM "Season" WHERE id = $1 FOR UPDATE', seasonId) as Array<{ id: string }>;
      if (rows.length === 0) await database.$executeRawUnsafe("SELECT pg_advisory_xact_lock(hashtextextended($1, 0))", seasonId);
    } else await database.$executeRawUnsafe("PRAGMA defer_foreign_keys = ON");
  }

  private async hostedDuplicate<T>(database: any, metadata: CommandMetadata): Promise<{ found: false } | { found: true; result: T }> {
    if (this.dialect !== "postgres") return { found: false };
    const receipt = await database.commandReceipt.findUnique({
      where: { seasonId_actorSubjectId_idempotencyKey: { seasonId: metadata.seasonId, actorSubjectId: metadata.actor.subjectId, idempotencyKey: metadata.idempotencyKey } },
    });
    if (!receipt) return { found: false };
    if (receipt.commandFingerprint !== this.commandFingerprint(metadata)) throw new Error("Idempotency key was already used for a different command");
    const stored = JSON.parse(receipt.resultJson) as { present: boolean; value: T | null };
    return { found: true, result: (stored.present ? stored.value : undefined) as T };
  }

  private async recordHostedCommit(database: any, metadata: CommandMetadata, auditId: string, correlationId: string, result: unknown, seasonRevision: number): Promise<void> {
    if (this.dialect !== "postgres") return;
    await database.commandReceipt.create({ data: { id: randomUUID(), seasonId: metadata.seasonId, actorSubjectId: metadata.actor.subjectId, idempotencyKey: metadata.idempotencyKey, commandType: metadata.commandType, commandFingerprint: this.commandFingerprint(metadata), resultJson: JSON.stringify({ present: result !== undefined, value: result ?? null }), auditEventId: auditId, seasonRevision } });
    await database.outboxEvent.create({ data: { id: randomUUID(), seasonId: metadata.seasonId, seasonRevision, projectionKind: "SEASON_INVALIDATED", commandType: metadata.commandType, correlationId } });
  }

  private transaction<T>(work: (database: any) => Promise<T>): Promise<T> {
    const client = this.prisma as unknown as { $transaction<R>(callback: (database: any) => Promise<R>, options?: { isolationLevel?: string }): Promise<R> };
    return client.$transaction(work, this.dialect === "postgres" ? { isolationLevel: "ReadCommitted" } : undefined);
  }
  ...
  execute<T>(metadata: CommandMetadata, operation: (transaction: SeasonTransaction) => T | Promise<T>): Promise<T> {
    const commit = this.dialect === "sqlite"
      ? this.queue.then(() => this.executeNow(metadata, operation))
      : this.retryPostgresTransaction(() => this.executeNow(metadata, operation));
    if (this.dialect === "sqlite") this.queue = commit.then(() => undefined, () => undefined);
  ...
  async hasExecutedCommand(actor: ActorDescriptor, seasonId: string, idempotencyKey: string) {
    if (this.dialect === "postgres") return (await (this.prisma as any).commandReceipt.count({ where: { seasonId, actorSubjectId: actor.subjectId, idempotencyKey } })) > 0;
  ...
  private async executeNow<T>(metadata: CommandMetadata, operation: (transaction: SeasonTransaction) => T | Promise<T>): Promise<{ result: T; notification?: CommitNotification }> {
    if (this.dialect === "postgres") {
      const duplicate = await this.hostedDuplicate<T>(this.prisma as any, metadata);
      if (duplicate.found) return { result: duplicate.result };
    } else {
    ...
    return this.transaction(async database => {
      await this.lockSeason(database, metadata.seasonId);
      const replay = await this.hostedDuplicate<T>(database, metadata);
      if (replay.found) return { result: replay.result };
      ...
      await database.auditEvent.create({ data: { id: auditId, seasonId: metadata.seasonId, sequence: (latest._max.sequence ?? 0) + 1, actorType: metadata.actor.type, actorLabel: metadata.actor.label, ...(this.dialect === "postgres" ? { actorSubjectId: metadata.actor.subjectId, actorRole: metadata.actor.effectiveRole } : {}), commandType: metadata.commandType, correlationId, idempotencyKey: metadata.idempotencyKey, reason: metadata.reason ?? null, beforeJson: before ? JSON.stringify(before) : null, afterJson: after ? JSON.stringify(after) : null, resultJson: JSON.stringify(result) } });
      await this.recordHostedCommit(database, metadata, auditId, correlationId, result, after?.rowVersion ?? 0);
      return {
        result,
        notification: {
  ...
  private setupCommand<T>(metadata: CommandMetadata, work: (database: Parameters<Parameters<PrismaClient["$transaction"]>[0]>[0], auditId: string) => Promise<T>, options: { submissionScoped?: boolean } = {}): Promise<T> {
    const executeCommand = async () => {
      if (this.dialect === "postgres") {
        const duplicate = await this.hostedDuplicate<T>(this.prisma as any, metadata);
        if (duplicate.found) return duplicate.result;
      } else {
        const duplicate = await this.prisma.auditEvent.findUnique({ where: { seasonId_actorType_idempotencyKey: { seasonId: metadata.seasonId, actorType: metadata.actor.type, idempotencyKey: metadata.idempotencyKey } }, select: { resultJson: true } });
      ...
      return this.transaction(async database => {
        ...
        const submissionCommand = this.dialect === "postgres" && options.submissionScoped === true && metadata.expectedSubmissionVersion !== undefined;
        if (!submissionCommand && metadata.expectedVersion !== undefined && beforeRow.rowVersion !== metadata.expectedVersion) throw new Error(`Stale season version: expected ${metadata.expectedVersion}, found ${beforeRow.rowVersion}`);
        ...
        await this.recordHostedCommit(database, metadata, auditId, correlationId, result, afterRow.rowVersion);
        return result;
      });
    };
    const run = this.dialect === "sqlite" ? this.queue.then(executeCommand) : this.retryPostgresTransaction(executeCommand);
    if (this.dialect === "sqlite") this.queue = run.then(() => undefined, () => undefined);
    return run;
  }
  ...
  async saveSubmission(metadata: CommandMetadata, roundNumber: AuctionRoundNumber, seasonTeamId: string, bids: AuctionBidDraft[], finalize: boolean, confirmZero: boolean): Promise<void> {
    if (this.dialect === "postgres" && metadata.expectedSubmissionVersion === undefined) throw new Error("Hosted submission writes require an expected submission version");
    await this.setupCommand(metadata, async database => {
      ...
      if (this.dialect === "postgres" && metadata.expectedSubmissionVersion !== undefined) {
        const changed = await (database.auctionSubmission as any).updateMany({ where: { id: submission.id, submissionVersion: metadata.expectedSubmissionVersion }, data: { bidsJson: JSON.stringify(encoded), bidCount: bids.length, status: finalize ? "FINAL" : "DRAFT", zeroConfirmed: bids.length === 0 && confirmZero, submissionVersion: { increment: 1 } } });
        if (changed.count !== 1) throw new Error(`Stale submission version: expected ${metadata.expectedSubmissionVersion}, found ${(submission as any).submissionVersion}`);
      } else {
        await database.auctionSubmission.update({ where: { id: submission.id }, data: { bidsJson: JSON.stringify(encoded), bidCount: bids.length, status: finalize ? "FINAL" : "DRAFT", zeroConfirmed: bids.length === 0 && confirmZero, ...(this.dialect === "postgres" ? { submissionVersion: { increment: 1 } } : {}) } });
      }
      await database.season.update({ where: { id: metadata.seasonId }, data: { rowVersion: { increment: 1 } } });
    }, { submissionScoped: true });
  }
  ...
  async finalizeSubmission(metadata: CommandMetadata, roundNumber: AuctionRoundNumber, seasonTeamId: string, confirmZero: boolean): Promise<void> {
    ...
    await this.setupCommand(metadata, async database => {
      ...
      if (this.dialect === "postgres" && metadata.expectedSubmissionVersion !== undefined) {
        const changed = await (database.auctionSubmission as any).updateMany({ where: { id: submission.id, submissionVersion: metadata.expectedSubmissionVersion }, data: { status: "FINAL", zeroConfirmed: submission.bidCount === 0 && confirmZero, submissionVersion: { increment: 1 } } });
        if (changed.count !== 1) throw new Error(`Stale submission version: expected ${metadata.expectedSubmissionVersion}, found ${(submission as any).submissionVersion}`);
      } else {
        await database.auctionSubmission.update({ where: { id: submission.id }, data: { status: "FINAL", zeroConfirmed: submission.bidCount === 0 && confirmZero, ...(this.dialect === "postgres" ? { submissionVersion: { increment: 1 } } : {}) } });
      }
      await database.season.update({ where: { id: metadata.seasonId }, data: { rowVersion: { increment: 1 } } });
    }, { submissionScoped: true });
  }
