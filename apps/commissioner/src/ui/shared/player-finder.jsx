...
export const boundedFinderPage = (requested, totalPages) => Math.min(requested, Math.max(totalPages, 1));
export function PlayerFinder({label="Player",seasonId,request,stagePolicy="SETUP",selectedPlayerId,onSelect,initialPage=null,refreshToken=0}){
  ...
  const load=async(nextPage=1)=>{
    ...
    try{
      ...
      const boundedPage = boundedFinderPage(nextPage, result.totalPages);
      if (result.total > 0 && boundedPage !== nextPage) { await load(boundedPage); return; }
      setPage({...result,page:boundedPage});
  ...
  useEffect(()=>{if(previousRefresh.current!==refreshToken){previousRefresh.current=refreshToken;void load(page.page);}},[refreshToken]);
