...
async function api(path, method = "GET", body) {
  ...
  if (data?.season?.rowVersion !== undefined) currentSeasonVersion = data.season.rowVersion;
  else if (method !== "GET") { const parts = path.split("?")[0].split("/"); const id = parts[2] === "setup" ? parts[3] : parts[3]; if (id && id !== "seasons" && id !== "corrections") { const summary = await fetch(`/api/setup/${id}`).then(result => result.json()); if (summary?.season?.rowVersion !== undefined) currentSeasonVersion = summary.season.rowVersion; } }
  return data;
}
...
function SetupApp() {
  ...
  return <main><header><p className="eyebrow">Local commissioner console</p><h1>Draft setup & sealed auctions</h1><p role="status">{message}</p></header><fieldset disabled={busy}>
    ...
    {seasonId && <section><h2>Auction round {roundNumber}</h2><p>Team contents remain masked until the round is locked and explicitly revealed.</p><button onClick={() => runAuction(() => api(`/api/auction/${seasonId}/${roundNumber}/open`, "POST"))}>Open round {roundNumber}</button>{auction?.roundNumber === roundNumber && <><label>Bid player ID <input aria-label="Bid player ID" value={bidPlayerId} onChange={event => setBidPlayerId(event.target.value)} /></label><label>Amount <input aria-label="Bid amount" type="number" min="1" value={bidAmount} onChange={event => setBidAmount(event.target.value)} /></label>{auction.teams.map(team => <span key={team.seasonTeamId}><button disabled={auction.status !== "BIDDING" || !bidPlayerId} onClick={() => runAuction(async () => { await api(`/api/auction/${seasonId}/${roundNumber}/teams/${team.seasonTeamId}`, "PUT", { bids: [{ playerId: bidPlayerId, amount: Number(bidAmount) }], finalize: true }); return refreshAuction(); })}>Finalize bid for {team.displayName}</button><button disabled={auction.status !== "BIDDING"} onClick={() => runAuction(async () => { await api(`/api/auction/${seasonId}/${roundNumber}/teams/${team.seasonTeamId}`, "PUT", { bids: [], finalize: true, confirmZero: true }); return refreshAuction(); })}>Finalize zero bids for {team.displayName}</button></span>)}</>}
      <button disabled={!auction || auction.status !== "BIDDING"} onClick={() => runAuction(async () => { await api(`/api/auction/${seasonId}/${roundNumber}/lock`, "POST", {}); return refreshAuction(roundNumber, true); })}>Lock, resolve & reveal round {roundNumber}</button>
