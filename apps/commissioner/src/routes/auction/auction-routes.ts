...
export async function registerAuctionRoutes(server: FastifyInstance, service: AuctionService) {
  ...
  server.get<{ Params: { seasonId: string; round: string; teamId: string } }>("/api/auction/:seasonId/:round/teams/:teamId/submission", request => service.submission(actor, request.params.seasonId, round(request.params.round), request.params.teamId));
  server.post<{ Params: { seasonId: string; round: string; teamId: string }; Body: { confirmZero?: boolean } }>("/api/auction/:seasonId/:round/teams/:teamId/finalize", async request => { await service.finalize(metadata(request, request.params.seasonId, "FINALIZE_AUCTION_SUBMISSION"), round(request.params.round), request.params.teamId, request.body); return service.summary(actor, request.params.seasonId, round(request.params.round)); });
