...
function metadata(request: FastifyRequest, seasonId: string, commandType: string): CommandMetadata {
  const key = request.headers["idempotency-key"];
  if (typeof key !== "string" || !key) {
    const error = new Error("Idempotency-Key header is required") as Error & { statusCode: number };
    error.statusCode = 400;
    throw error;
  }
  return { actor, seasonId, commandType, idempotencyKey: key };
}
...
export async function registerSetupRoutes(server: FastifyInstance, service: SetupService) {
  server.post<{ Body: { seasonId?: string; leagueId: string; year: number; name: string; teamCount: number } }>("/api/setup/seasons", async request => {
    ...
    return service.createSeason(metadata(request, seasonId, "CREATE_SEASON"), request.body);
  });
  ...
  server.post<{ Params: { seasonId: string }; Body: { namespace: string; content: string; format: "csv" | "json" } }>("/api/setup/:seasonId/imports/preview", request => service.previewImport(metadata(request, request.params.seasonId, "PREVIEW_IMPORT"), request.body.namespace, request.body.content, request.body.format));
