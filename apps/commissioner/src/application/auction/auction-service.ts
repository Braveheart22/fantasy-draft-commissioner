...
export class AuctionService {
  ...
  async open(metadata: CommandMetadata, round: AuctionRoundNumber) { const result=await this.repository.openRound(metadata, round);return{...result,rowVersion:await this.repository.seasonVersion(metadata.actor,metadata.seasonId)}; }
  ...
  async lockAndResolve(metadata: CommandMetadata, round: AuctionRoundNumber, rosterRules: CommissionerAuctionInput["rosterRules"]) {
    ...
    return { ...result, rowVersion: await this.repository.seasonVersion(metadata.actor,metadata.seasonId) };
  }
  ...
  async summary(actor: ActorDescriptor, seasonId: string, round: AuctionRoundNumber, reveal = false) { const result=await this.repository.summary(actor, seasonId, round, reveal);return{...result,rowVersion:await this.repository.seasonVersion(actor,seasonId)}; }
}
