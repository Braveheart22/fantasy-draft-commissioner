...
export class ConventionalDraftService {
  ...
  async pick(metadata: CommandMetadata, input: DraftPickInput) { const summary=await this.repository.draftSummary(metadata.actor,metadata.seasonId);if(summary.nextOverallPick===1)await this.checkpoints?.before(metadata,"PRE_CONVENTIONAL_DRAFT_START");const result=await this.repository.makePick(metadata,input);return{...result,rowVersion:await this.repository.seasonVersion(metadata.actor,metadata.seasonId)}; }
  async summary(metadata: Pick<CommandMetadata, "actor" | "seasonId">) { const result=await this.repository.draftSummary(metadata.actor, metadata.seasonId);return{...result,rowVersion:await this.repository.seasonVersion(metadata.actor,metadata.seasonId)}; }
}
